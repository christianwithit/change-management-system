...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Check if user has access to Development (IT, Admin, or Head of Tech)
  if (user.role !== "it" && user.role !== "admin" && user.role !== "headoftech") {
    alert(
      "Access denied. This page is only for IT personnel, Head of Technology, and Administrators.",
    );
...
function initializeEventListeners() {
  ...
  if (startDateInput)
    startDateInput.addEventListener("change", calculateDeadline);
  ...
  if (logAttachment) {
    logAttachment.addEventListener("change", handleFileSelect);
  }
...
function handleFileSelect(e) {
  ...
  if (file.size > 5 * 1024 * 1024) {
    utils.showAlert("File size exceeds 5MB limit", "warning");
    resetAttachment();
    return;
  }
  ...
  if (fileNameDisplay && attachmentNameDiv) {
...
function resetAttachment() {
  ...
  if (logAttachment) logAttachment.value = "";
  if (fileNameDisplay) fileNameDisplay.textContent = "";
  if (attachmentNameDiv) attachmentNameDiv.classList.add("hidden");
}
...
function loadProjects() {
  ...
  allProjects = allRequests.filter(
    (r) => r.itDecision === "Accepted" || r.developmentStatus,
  );
  ...
  allProjects.forEach((project) => {
    ...
    if (project.documentationUploaded === undefined)
      project.documentationUploaded = false;
    if (project.trainingConducted === undefined)
      project.trainingConducted = false;
    if (project.solutionDelivered === undefined)
      project.solutionDelivered = false;
    if (project.completionReportGenerated === undefined)
      project.completionReportGenerated = false;
  });
...
function updateStatistics() {
  ...
  const onHold = allProjects.filter(
    (p) => p.developmentStatus === "On Hold",
  ).length;
  const completed = allProjects.filter(
    (p) => p.developmentStatus === "Completed",
  ).length;
                ...
                <div class="mb-4 bg-white p-4 rounded-lg border border-gray-200">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button data-action="open-milestones-modal" data-project-id="${project.id}"
                            class="text-xs text-visionRed hover:text-red-700 font-medium">Manage</button>
                    </div>
...
function submitStatusUpdate() {
  ...
  const daysSinceStart = calculateDaysElapsed(
    currentProject.developmentStartDate || updates.developmentStartDate,
  );
...
function openMilestonesModal(projectId) {
  ...
  // Set checkbox states
  document.getElementById("milestone_documentation").checked =
    currentProject.documentationUploaded || false;
  document.getElementById("milestone_training").checked =
    currentProject.trainingConducted || false;
  document.getElementById("milestone_solution").checked =
    currentProject.solutionDelivered || false;
  document.getElementById("milestone_report").checked =
    currentProject.completionReportGenerated || false;
...
function submitMilestones() {
  ...
  const updates = {
    documentationUploaded: document.getElementById("milestone_documentation")
      .checked,
  ...
  if (
    updates.completionReportGenerated &&
    !currentProject.completionReportGenerated
  ) {
...
function openLogsModal(projectId) {
  ...
  modalInfo.innerHTML = `
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p class="text-sm text-gray-600 mb-1">Project ID: <strong class="text-gray-800">${currentProject.id}</strong></p>
            <p class="font-bold text-gray-800">${currentProject.title}</p>
            <p class="text-sm text-gray-600 mt-2">Started: ${currentProject.developmentStartDate ? utils.formatDate(currentProject.developmentStartDate) : "Not started"} • ${calculateDaysElapsed(currentProject.developmentStartDate)} days elapsed</p>
        </div>
    `;
                            ...
                            <div>
                                ...
                                ${
                                  log.previousStatus && log.newStatus
                                    ? `
                                    <span class="text-sm text-gray-600 ml-2">${log.previousStatus} → ${log.newStatus}</span>
                                `
                                    : ""
                                }
                            </div>
                        ...
                        ${
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-100 flex items-center gap-2">
                                <div class="w-8 h-8 rounded bg-gray-100 flex items-center justify-center text-gray-500">
                                    <i class="ph ${log.attachment.type.includes('image') ? 'ph-image' : 'ph-file-text'} text-lg"></i>
                                </div>
                                <div class="flex-1 overflow-hidden">
                                    <div class="text-xs font-semibold text-gray-800 truncate">${log.attachment.name}</div>
                                    <div class="text-[10px] text-gray-500">${(log.attachment.size / 1024).toFixed(1)} KB</div>
                                </div>
                                <a href="${log.attachment.data}" target="_blank" download="${log.attachment.name}"
                                    class="px-3 py-1 bg-white hover:bg-gray-50 text-visionRed border border-gray-200 rounded text-xs font-medium transition-all flex items-center gap-1">
...
function submitLogEntry() {
  ...
  const fileInput = document.getElementById("logAttachment");
  ...
  const daysSinceStart = calculateDaysElapsed(
    currentProject.developmentStartDate,
  );
  ...
  function saveLog(data) {
    API.addDevelopmentLog(currentProject.id, data);
...
function renderTimeline() {
  ...
  let timelineProjects = filteredProjects.filter(
    (p) => p.timelineStartDate && p.timelineDuration,
  );
  ...
  const warnings = window.checkDueDateNotifications
    ? window.checkDueDateNotifications()
    : [];
  ...
  if (timelineWarnings.length > 0) {
    const criticalCount = timelineWarnings.filter(
    ...
    html += `
            <div class="bg-amber-50 border-l-4 border-amber-500 rounded-lg shadow-sm p-4 mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1">
                        <h3 class="text-base font-bold text-slate-900">${timelineWarnings.length} Task${timelineWarnings.length !== 1 ? "s" : ""} Need${timelineWarnings.length === 1 ? "s" : ""} Attention</h3>
                        <p class="text-sm text-slate-700 mt-1">${criticalCount > 0 ? `${criticalCount} overdue, ` : ""}${timelineWarnings.length - criticalCount} due soon</p>
                    </div>
                </div>
            </div>
        `;
...
function renderTimelineProjects(projects, dates, totalUnits, gridCols, today) {
  ...
  projects.forEach((project) => {
    ...
    const warning = window.getTaskWarningStatus
      ? window.getTaskWarningStatus(project.timelineDeadline)
      : { text: "" };
    ...
    html += `
            <div class="timeline-task-row">
                ...
                <div class="timeline-task-canvas" style="grid-template-columns: ${gridCols};">
                    ...
                    <div class="timeline-pill ${statusClass}" data-project-id="${project.id}"
...
function calculateGridPosition(startDate, endDate, dates, zoom) {
  ...
  if (zoom === "day") {
    ...
    const endDayIndex = Math.floor(
      (endDate - earliestDate) / (1000 * 60 * 60 * 24),
    );
